...
73 checks on the content layer. Runs the real `content/*.js` and `js/content.js`
...
- the Stripe checkout functions use server-side secrets and a server catalog
...
- change detection queues only the files that actually differ, including the
  server Stripe catalog when Shop changes
...
253 checks. Renders each real page in jsdom, runs every one of the site's
...
faking success, the combined Stripe cart, and the dashboard — every screen
rendering, and an edit turning into valid publishable files.
